---
title: Guide to Configuring Plug-ins
author: 
  - Jason van Zyl
  - Vincent Siveton
date: 2009-08-26
...
All plugins should have at least the minimal required [information](/ref/current/maven\-model/maven\.html\#class\_plugin): `groupId`, `artifactId` and `version`\.
...
1. If the first item XML element contains a `.`, try to load the class with the fully qualified class name given in the element name
1. Try the first item XML element name \(with capitalized first letter\) as a class in the same package as the mojo/object being configured
