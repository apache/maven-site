...
7. Git Credential helpers (`git-credential:<helper-name>?url=<url>`), uses a [git credential helper binary](https://git-scm.com/docs/gitcredentials) to retrieve the password. The given `<url>` must be an absolute URL (consisting out of `https://`, some host and a path) uniquely identifying the necessary password (it does not need to resolve or return something). There is a list of common helpers at <https://git-scm.com/doc/credential-helpers>.
