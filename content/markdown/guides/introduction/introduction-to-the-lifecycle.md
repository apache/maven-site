...
- [Build Lifecycle Basics](Build\_Lifecycle\_Basics)
- [Setting Up Your Project to Use the Build Lifecycle](Setting\_Up\_Your\_Project\_to\_Use\_the\_Build\_Lifecycle)
    - [Packaging](Packaging)
    - [Plugins](Plugins)
- [Lifecycle Reference](Lifecycle\_Reference)
- [Built\-in Lifecycle Bindings](Built\-in\_Lifecycle\_Bindings)
...
For example, the default lifecycle comprises of the following phases \(for a complete list of the lifecycle phases, refer to the [Lifecycle Reference](Lifecycle\_Reference)\):
...
This is an almost [standard set of bindings](/ref/current/maven\-core/default\-bindings\.html); however, some packagings handle them differently\. For example, a project that is purely metadata \(packaging value is `pom`\) only binds goals to the `install` and `deploy` phases \(for a complete list of goal\-to\-build\-phase bindings of some of the packaging types, refer to the [Lifecycle Reference](Lifecycle\_Reference)\)\.
